...
\newcommand{\keyxvar}{\mathit{x}}
\newcommand{\keyyvar}{\mathit{y}}
\newcommand{\keyzvar}{\mathit{z}}
\newcommand{\Val}{{\sf Val}}
\newcommand{\valvar}{\mathit{v}}
...
\newcommand{\starttime}[3]{
	\draw[start] ($(#1.west) + (0, #2)$) to ($(#1.west) + (0, -#3)$);}
\newcommand{\committime}[3]{
	\draw[commit] ($(#1.east) + (0, #2)$) to ($(#1.east) + (0, -#3)$);}
\newcommand{\startcommittime}[3]{
	\starttime{#1}{#2}{#3}
	\committime{#1}{#2}{#3}}

\newcommand{\tsorder}[4] {
	\node[start, font = \Large] at ($(#1.west) + (0, #2)$) {\ding{#3}};
	\node[commit, font = \Large] at ($(#1.east) + (0, #2)$) {\ding{#4}};
}