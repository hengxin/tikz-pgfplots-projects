...
\begin{tikzpicture}[scale=1,    every axis/.style={
	...
	\begin{axis}[legend style={draw=none,at={(1, 0.7)}, anchor=west},axis on top]
		\addplot[color=blue, pattern color=blue, pattern=north east lines] table [x expr=\coordindex, y=mini_time, col sep=comma] {./data/si-checker-keydist.csv};
		\addplot[color=blue, pattern color=blue, pattern=crosshatch dots] table [x expr=\coordindex, y=polysi_time, col sep=comma] {./data/si-checker-keydist.csv};
		\legend{Mini-SI, PolySI}
	\end{axis}
\end{tikzpicture}
