...
\begin{tikzpicture}[
  ...
  % t4-WR(x)-t5
  \draw[wr,orange] (t4) to % node[above]{\textcolor{lightgray}{$\SO$}}
    node[below]{$\WR(x)$} (t5);
  % t4-WR-t1
  ...
  % t2-CO-t5
  \draw[co,bend left = 20, orange] (t2) to node[sloped, above, very near end]{$\CO$} (t5);
  % t4-WR-t3
  ...
  % t4-VO-t2
  \draw[vo, orange, bend left] (t4.160) to node[]{$\CM$} (t2.-160);
\end{tikzpicture}
\end{document}