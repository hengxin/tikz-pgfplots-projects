...
\begin{tikzpicture}[
  ...
  \node[txn, label = left : Monica:, label = 60 : $t_\mathit{Mr}$, below right = 1.20cm and -1.00cm of joey, anchor = center]
  ...
  % joey-wr-monica-read
  \draw[wr] (joey) to node[sloped, above]{$\WR$} (monica-read);
  ...
  % joey-co-rachel
  \draw[wr, out = -120, in = 180, red] (joey.-170) to node[sloped, above, pos = 0.60, red]{$\CO$} (rachel);
\end{tikzpicture}
\end{document}