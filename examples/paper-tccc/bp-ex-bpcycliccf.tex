...
\begin{tikzpicture}[
  ...
  % t5: R(y, 1)
  \node[txn, below = 2.00cm of t3, label = below : $t_5$]
  ...
  % t4-WR(x)-t5
  \draw[wr] (t4) to % node[above]{\textcolor{lightgray}{$\SO$}}
    node[below]{$\WR(x)$} (t5);
  % t4-WR-t1
  ...
  % t2-CO-t5
  \draw[co, bend left, red] (t2) to node[sloped, above]{$\CO$} (t5);
  % t4-WR-t3
  ...
  % t4-VO-t2
  \draw[vo, red, bend left] (t4.160) to node[]{$\AO$} (t2.-160);
\end{tikzpicture}
\end{document}