...
\begin{tikzpicture}[
  ...
  \node[txn, label = left : Joey:, label = above : $t_{J}$]
    (joey) {$\writeevent(\mathit{start}, \textrm{Los Angeles})$};
  \node[txn, label = left : Monica:, label = 60 : $t_{Mr}$, below right = 1.20cm and -1.00cm of joey, anchor = center]
  ...
  % joey-vis-monica-read
  \draw[wr] (joey) to node[sloped, above]{$\VIS$} (monica-read);
  ...
  % joey-vis-rachel
  \draw[wr, out = -120, in = 180, red] (joey.-170) to node[sloped, above, pos = 0.60, red]{$\VIS$} (rachel);
\end{tikzpicture}
\end{document}