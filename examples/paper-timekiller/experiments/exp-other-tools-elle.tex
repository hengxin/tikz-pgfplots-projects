\documentclass{standalone}
\input{../newcommands}

\input{pgfplotsset}

\begin{document}
	\begin{tikzpicture}[]
		\begin{axis}[
			xlabel={\#txns},
			ylabel={Time (s)},
			ymax=9,
			ymin=0,
			legend pos=north west,
			xtick={0,2000,4000,6000,8000,10000},
			xticklabels={0,2,4,6,8,10},
			xtick scale label code/.code={$\cdot 10^{3}$},
			cycle multiindex* list={
				color       \nextlist
				mark list*  \nextlist
			}]
			\addplot[color=brown,mark=o,mark size=3pt] table [x=txns, y=ElleList, col sep=comma] {./data/other-tools-elle.csv};
			\addplot[color=black,mark=x,mark size=3pt] table [x=txns, y=TimeKiller, col sep=comma] {./data/other-tools-elle.csv};
			\addplot[color=brown,mark=o,mark size=3pt,nodes near coords,node near coord style={font=\scriptsize}] coordinates {(10000,7.108)};
			\addplot[color=black,mark=x,mark size=3pt,nodes near coords,node near coord style={font=\scriptsize}] coordinates {(10000,0.961)};
			\legend{ElleList,\textup{\textsc{Chronos}}}
		\end{axis}
	\end{tikzpicture}
\end{document}