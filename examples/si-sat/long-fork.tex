...
\begin{tikzpicture}[
  ...
  \node[txn, label = left : $T_{1}$, above = of t0] (t1)
    {$\writeevent(\keyxvar, 1)$};
  \node[txn, label = left : $T_{2}$, below = of t0] (t2)
  ...
  % t0 WR(y) t3
  \draw[wr] (t0) to node[above, sloped]{$\WR(\keyyvar)$} (t3);
  % t0 WR(x) t4
  \draw[wr] (t0) to node[above, sloped]{$\WR(\keyxvar)$} (t4);
  % t1 WR(x) t3
