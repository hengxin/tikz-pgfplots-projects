...
\begin{tikzpicture}[seq/.style = {rectangle split, 
  ...
  \draw[->, >=Stealth, bend left = 45, purple] (x) to node[right, near end] {\texttt{push(X, Q)}} (queue.east);
  \draw[->, >=Stealth, bend left = 45, purple] (queue.west) to node[left, near start] {\texttt{pop(X, Q)}} (x);
\end{tikzpicture}
\end{document}