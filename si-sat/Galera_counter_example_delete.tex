...
\begin{document}
    ...
    \begin{tikzpicture}[model/.style = {draw, minimum size = 15pt},  node distance = 0.5cm and 1.5cm]
        ...
        \path[dashed, -latex, color=red] (15.south) edge node[below] {$\RW$} (213.east); 
        \path[dashed, -latex, color=blue] ([xshift=5ex]213.north) edge node[above] {$\RW$} ([yshift=-1ex]15.west); 
        %\path[dashed, -latex, color=blue] ([yshift=-1ex]15.west) edge [out=180,in=90] node[below] {$\RW$} ([xshift=5ex]213.north);
        ...
        \path[-latex, color=red] ([yshift=2ex]14.east) edge node[above] {$\WW$} ([yshift=2ex]15.west);
        \path[-latex] (14.south) edge node[right] {$\WR$} (213.north);
        \path[-latex,color=blue] ([xshift=-2ex]14.south) edge node[left] {$\WW$} ([xshift=-2ex]213.north);
