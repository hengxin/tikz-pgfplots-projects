...
\begin{tikzpicture}[
  ...
  \node[txn, below = of 4-167.center, label = above left : {$(7, 320)$}] (7-320) {$\readevent(\keyvar_{761}, 5)$ \\ $\writeevent(\keyvar_{23}, 6)$};
  ...
  \path (10-470) edge[so] node[above]{$\SO$} (10-471)
	...
	(4-167) edge[wr] node[]{$\WR(\keyvar_{761})$} (7-320)
	...
	(4-167) edge[so, red, very thick] node[above]{$\SO$} (4-178);
  \end{tikzpicture}
\end{document}