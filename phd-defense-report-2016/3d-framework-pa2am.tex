...
\draw[thick, >=Stealth, ->] (O) to node[anchor = north, sloped, below, near end, align = center] {Measurement\\{\tiny (度量: \textcolor{red}{Quantifying})}} (y);
\draw[thick, >=Stealth, ->] (O) to (z) node[anchor = south, align = center] {Semantics\\{\tiny (模型: \textcolor{red}{(2-)Atomicity})}};
...
\node[canvas is yz plane at x = 0, align = center] at (0,0.5,0.5) {精细化,\\可度量\\(\textcolor{red}{PA2AM})};

\node[font = \normalsize, rotate = -90, teal] at (0,1.4,1.) {\textsc{R/W Register}};
\end{tikzpicture}
\end{document}
