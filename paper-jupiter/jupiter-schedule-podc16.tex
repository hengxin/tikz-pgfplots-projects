...
\newcommand{\csend}[4]{% #1: client; #2: client pos; #3: server pos; #4: seq. number
  \draw[->]  ($(#1)!#2!(#1\b)$) node[seq = 0.40] (#4) {#4} to ($(\s)!#3!(\s\b)$) node[seq = 0.60] {$#4$};
  % \send{#1}{\s}{#2}{#3}{#4}
}
...
\begin{tikzpicture}[timeline/.style = {very thick}, >=Stealth, 
  ...
  \node[replica] (\s) {$S$ \\ $(\epsilon)$};
  \node[replica, right = of s] (c1) {$C_1$ \\ $(\epsilon)$};
  \node[replica, right = of c1] (c2) {$C_2$ \\ $(\epsilon)$};
  \node[replica, right = of c2] (c3) {$C_3$ \\ $(\epsilon)$};
