...
\begin{tikzpicture}
  ...
  \path[path = {red}] ($(0.north) + (135:20pt)$) node[above left] (sc1) {$S, C_1$} -- ++(-90:2.5*\vdist) -- ++(-135:2.5*\vdist) -- ++(-42:2.5*\vdist) -- ++(-42:2.8*\vdist);
  \path[path = {blue}] ($(0.north) + (100:25pt)$) node[above] (c2) {$C_2$} -- ++(-90:2.7*\vdist) -- ++(-45:2.5*\vdist) -- ++(-135:2.3*\vdist) -- ++(-42:2.9*\vdist);
  \path[path = {teal}] ($(0.north) + (40:20pt)$) node[above right] (c3) {$C_3$} -- ++(-90:2.5*\vdist) -- ++(-22:4.5*\vdist) -- ++(-128:4.6*\vdist);
\end{tikzpicture}
\end{document}