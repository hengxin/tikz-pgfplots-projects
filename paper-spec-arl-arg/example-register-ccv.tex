...
\newcommand{\po}[2]{\draw [->, thick] (#1) to (#2);}
...
%  vis/.style = {->, shorten >= 3pt, very thick, dashed}
  ...
  \node (wy1) [wop, right = 0.5cm of wx1] {\texttt{W(y)1}};
  \node (wz1) [wop, right = 0.5cm of wy1] {\texttt{W(z)1}};
  
  \node (p2) [process, below = 1.0cm of p1] {$p_2:$};
  \node (rx2) [rop, right = 6cm of p2] {\texttt{R(x)2}};
  \node (rz1) [rop, right = 0.5cm of rx2] {\texttt{R(z)1}};
  \node (ry2) [rop, right = 0.5cm of rz1] {\texttt{R(y)2}};

  \node (p3) [process, below = 1.0cm of p2] {$p_3:$};
  \node (wy2) [wop, right = 0.4cm of p3] {\texttt{W(y)2}};
  \node (rx0) [rop, right = 0.5cm of wy2] {\texttt{R(x)0}};
  \node (wx2) [wop, right = 0.5cm of rx0] {\texttt{W(x)2}};
