...
\tikzset{
  ...
  \node (cc) [lconsistency, above right=0.8cm and 1.0cm of wcc] {%\large CC\\
                             \red{$so\subseteq vis$}\\
                             \teal{$ar = \emptyset$}\\
                             \blue{$V[e] = so^{-1}(e)$}};
  \node (ncc) [cn, below = 0.1cm of cc] {\texttt{CC}};

  \node (scc) [sconsistency, above right = 0.2cm and 1.5cm of cc] {%\large SCC\\
                             \red{$so\subseteq vis$}\\
                            \teal{$ar = \emptyset$}\\
                            \blue{$V[e] = E_H$}};
  \node (nscc) [cn, below = 0.1cm of scc] {\texttt{SCC}};

  \node (ccv) [lconsistency, right = 5.5cm of wcc] {%\large CCv\\
                             \red{$so\subseteq vis$}\\
                             \teal{$ar=\text{TO}(E_H)$}\\
                             \blue{$V[e]=so^{-1}(e)$}};
  ...
  \node (wccv) [sconsistency, below right=1.5cm and 2.0cm of wcc] {%\large WCCv\\
                             \red{$so\subseteq vis$}\\
                             \teal{$ar = \text{TO}(E_H)$}\\
                             \blue{$V[e] =\emptyset$}};
