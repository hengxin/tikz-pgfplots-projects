...
\newcommand{\vis}[2]{\draw [->, thick, dashed] (#1) to node[above, sloped, near end] {\Large{rf}} (#2);}
...
\tikzset{
  ...
  \node (rx2) [rop, right = 2cm of wx1] {$x.\rd\triangleright2$};

  \node (p2) [process, below = 1.2cm of p1] {$p_2:$};
  \node (wx2) [wop, right = 0.6cm of p2] {$x.\wr(2)$};
  \node (rx1) [rop, right = 2cm of wx2] {$x.\rd\triangleright1$};
