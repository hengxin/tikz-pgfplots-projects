...
\begin{tikzpicture}[start chain = x going right,
  ...
  % k1
  \begin{scope}
    \draw let \p{p-x2} = (x2.east), \p{p-ti} = (ti.south west) in
      node (c-x2) [blue] at ($(\x{p-x2}, \y{p-ti})-(0.0cm, 2.0cm)$) {$c_{x_2}$};

      \draw[dashed] (x2.south east) to (c-x2.north);
    % \draw[dashed] let \p{p-x2} = (x2.east), \p{p-ti} = (ti.south west) in
    %   (x2.south east) to ($(\x{p-x2}, \y{p-ti})-(0.0cm, 1.0cm)$);
    % \draw[dashed] (ti.south west) to ($(ti.south west)-(0.0cm, 1.0cm)$);
  \end{scope}
\end{tikzpicture}
