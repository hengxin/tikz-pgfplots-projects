...
\begin{scope}[master/.style = {rectangle, rounded corners, font = \LARGE, fill = blue!40, inner sep 
  ...
  \node (dist-tx-text) [font = \LARGE, blue, above = 1.00cm of master-surrounding.north] {distributed transaction};
\end{scope}
...
\begin{scope}[slave/.style = {rectangle, rounded corners, font = \LARGE, fill = blue!40, inner sep = 
  ...
  \node (slave-left-above) [slave, below left = 0.5cm and 1.0cm of master-left] {Slave-1-1};
  \node (slave-left-below) [slave, below = 1.5cm of slave-left-above] {Slave-1-2};
  \draw [rep] (master-left) to (slave-left-above);
  \draw [rep] (master-left) to node[sloped, font = \LARGE, below] {replication} (slave-left-below);
  % slaves of master-right
  \node (slave-right-above) [slave, below right = 0.5cm and 1.0cm of master-right] {Slave-2-1};
  \node (slave-right-below) [slave, below = 1.5cm of slave-right-above] {Slave-2-2};
  \draw [rep] (master-right) to (slave-right-above);
  \draw [rep] (master-right) to node[sloped, font = \LARGE, below] {replication} (slave-right-below);
\end{scope}
